import { MODEL_RISK_RADAR_CONTRACT_ADDRESS } from "@/lib/genlayer";

const repoUrl = "https://github.com/klopp78/modelriskradar-genlayer";
const studioUrl = `https://explorer-studio.genlayer.com/address/${MODEL_RISK_RADAR_CONTRACT_ADDRESS}`;
    ...
    <main className="min-h-screen bg-[#f4f6f2] text-[#161814]">
          ...
          <div className="mt-7 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
            <div>
              ...
              <p className="mt-5 max-w-3xl text-lg leading-8 text-[#596452]">
                A consensus radar for AI model and API risk. Register an
                announcement, incident, or policy change, bind independent
                evidence snapshots, and ask GenLayer validators for a durable risk receipt.
              </p>
              <div className="mt-7 flex flex-wrap gap-3">
                <a className="action-button primary" href="/case">Register signal</a>
                <a className="action-button" href="/assess">Assess risk</a>
            ...
            <div className="escrow-board">
              ...
              <div>
                ...
                <strong>Each mrs_* signal and mrv_* risk verdict stays readable on-chain</strong>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-5 px-5 py-8 md:grid-cols-3 lg:px-8">
        <article className="tool-panel">
          ...
          <h2 className="mt-2 text-2xl font-semibold">Source-bound signal</h2>
          <p className="mt-3 leading-7 text-[#596452]">
            The contract stores the model change claim, subject URL, and
            SHA-256 commitments for every fetched evidence snapshot before assessment begins.
          </p>
          <a className="mt-5 inline-block text-sm font-semibold text-[#25614b]" href="/case">Open signal flow</a>
        </article>
        <article className="tool-panel">
          <span className="field-label">02 Assess</span>
          <h2 className="mt-2 text-2xl font-semibold">Consensus risk verdict</h2>
          <p className="mt-3 leading-7 text-[#596452]">
        ...
        <article className="tool-panel">
          ...
          <p className="mt-3 leading-7 text-[#596452]">
            Anyone can read a signal or verdict ID and inspect exactly which
      ...
      <footer className="mx-auto flex max-w-6xl flex-wrap gap-4 px-5 pb-10 text-sm text-[#596452] lg:px-8">
        ...
        <code>{MODEL_RISK_RADAR_CONTRACT_ADDRESS}</code>
      </footer>
    </main>
  );
}
