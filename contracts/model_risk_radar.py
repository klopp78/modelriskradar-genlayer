...
class RiskVerdict(typing.NamedTuple):
    decision: str
    confidence: u8
    subject_match: bool
...
class ModelRiskRadar(gl.Contract):
    """Consensus registry for AI model change and incident risk signals."""
    ...
    def __init__(self):
        self.signal_count = u64(0)
        self.latest_signal_id = ""
        self.latest_verdict_id = ""

    @gl.public.view
    def get_signal_count(self) -> u64:
        return self.signal_count

    @gl.public.view
    def get_latest_signal_id(self) -> str:
        return self.latest_signal_id
    ...
    @gl.public.view
    def list_signal_ids(self) -> str:
        return json.dumps([signal_id for signal_id in self.signal_ids], separators=(",", ":"))
    ...
    def register_signal(
        ...
        signal_id = _signal_id(reporter, normalized_claim, baseline["baseline_hash"])
        if len(self.signals.get(signal_id, "")) > 0:
            raise Exception("signal_already_registered")

        self.signal_count = u64(int(self.signal_count) + 1)
        record = {
            ...
            "verdict_ids": [],
            "state": "registered",
            "created_sequence": int(self.signal_count),
        }
        self.signals[signal_id] = _canonical_json(record)
        self.signal_ids.append(signal_id)
        self.latest_signal_id = signal_id
        return signal_id

    @gl.public.write
    def assess_signal(self, signal_id: str) -> str:
        case_record = _load_json(self.signals.get(signal_id, ""), "signal_not_found")
        if case_record["state"] not in ("registered", "assessed"):
            raise Exception("signal_not_assessable")

        def leader_fn():
            return _assess_model_risk(case_record)
        ...
        def validator_fn(leader_result) -> bool:
            ...
            try:
                ...
                independent = _parse_verdict(_assess_model_risk(case_record))
            except Exception:
                return False
            return (
        ...
        verdict_id = _verdict_id(signal_id, verdict["decision"], verdict["evidence_bundle_hash"])
        if len(self.verdicts.get(verdict_id, "")) > 0:
            raise Exception("risk_report_already_recorded")
        ...
        self.signals[signal_id] = _canonical_json(case_record)
...
def _assess_model_risk(case_record: dict) -> str:
    ...
    prompt = f"""
You are a GenLayer validator assessing AI model, API, or safety-policy risk evidence.
...
- Compare the subject announcement or incident page with primary evidence, archive, and independent context.
- subject_match is true only if the evidence is about the submitted model/API/safety change.
- evidence_diverse is true only if the evidence uses at least two distinct hosts and supports the claim.
- provenance_verified is true only if archive/context corroborates the primary evidence.
- Use risky for material capability changes, deprecations, exploit disclosures, policy regressions,
  conflicting official statements, or provenance failure.
- Use trusted when sources consistently support a low-impact or well-documented change.
...
def _parse_verdict(raw_json: str) -> RiskVerdict:
...
def _signal_id(reporter: str, claim: str, baseline_hash: str) -> str:
    return "mrs_" + _sha256(reporter + "|" + claim.lower() + "|" + baseline_hash)[:20]


def _verdict_id(signal_id: str, decision: str, evidence_hash: str) -> str:
    return "mrv_" + _sha256(signal_id + "|" + decision + "|" + evidence_hash)[:20]
